    ...
    object ModelUpdateManager {
        ...
        suspend fun checkForModelUpdates(context: Context, listener: UpdateListener? = null): Boolean {
            return withContext(Dispatchers.IO) {
                try {
                    ...
                    val remoteVersion = remote.optString("yolo_version", "")
                    val localJson = JSONObject(localVersion ?: "{}")
                    val localVersion2 = localJson.optString("yolo_version", "")

                    if (localVersion2 == remoteVersion) {
                        // already up to date
        ...
        fun hasDownloadedModels(context: Context): Boolean {
            val requiredFiles = listOf(
                "ml/yolov11_3_5.tflite",
                "ml/sweetpotato_v3_5.tflite",
                "ml/labels.txt",
                "ml/yolo_labels.json",
                "ml/scenario_classifier.tflite",
                "ml/scenario_labels.json",
                "ml/feature_scaler.json",
                "ml/label_maps.json",
